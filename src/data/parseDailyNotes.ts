...
//   let fileContents = await app.vault.cachedRead(note);
//   //eslint-disable-next-line
//   const matchLength = (
//     fileContents.match(/(-|\*) (\[(\s|x|X|\\|\-|\>|D|\?|\/|\+|R|\!|i|B|P|C)\]\s)?((\<time\>)?\d{1,2}\:\d{2})?/g) || []
//   ).length;
//   fileContents = null;
//   return matchLength;
// }
...
//   /^\s*[\-\*]\s(\[(\s|x|X|\\|\-|\>|D|\?|\/|\+|R|\!|i|B|P|C)\]\s?)?(\<time\>)?((\d{1,2})\:(\d{2}))?(\<\/time\>)?(.*)$/.exec(
//     line,
//   )?.[8];
// //eslint-disable-next-line
