...
// export async function getTasksFromFiles(file: TFile | null, dailyEvents: any[]): Promise<any[]> {
//   if (!file) {
//     return [];
//   }
//   const {vault} = dailyNotesService.getState().app;
...
//   /^(\s*)?(\-|\*)\s\[(\s|x|X|\\|\-|\>|D|\?|\/|\+|R|\!|i|B|P|C)\]\s?(\<time\>)?((\d{1,2})\:(\d{2}))?(\<\/time\>)?(.+?)(?=(⏫|🛫|📅|📆|(@{)|⏳|⌛|(\[due\:\:)|(\[created\:\:)|➕))/.exec(
//     line,
//   )?.[9];
// //eslint-disable-next-line
...
// //eslint-disable-next-line
// const getStartDateFromBulletLine = (line: string) =>
//   /\s(🛫|(\[created\:\:)|➕) ?(\d{4}-\d{2}-\d{2})(\])?/.exec(line)?.[3];
// //eslint-disable-next-line
// const getDueDateFromBulletLine = (line: string) =>
//   /\s(📅|📆|(@{)|(\[due\:\:)) ?(\d{4}-\d{2}-\d{2})(\])?/.exec(line)?.[4];
// //eslint-disable-next-line
